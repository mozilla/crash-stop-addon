...
def get_filter_query(fa_bids, fx_bids):
    ...
    return {'product': ['FennecAndroid', 'Firefox'],
            'date': '>=' + date,
            'build_id': '>=' + min_bid,
...
def filter_nightly_buildids_helper(fa_bids, fx_bids):
    ...
    socorro.SuperSearch(params=params, handler=filter_by_crashes_num, handlerdata=data).wait()
...
def filter_signatures_data(limit, product, platforms, sgn, bids_chan, json, data):
    ...
    for facets in json['facets']['build_id']:
        ...
        if bid in index:
            ...
            installs[n] = max(limit, facets['cardinality_install_time']['value']) if N == limit else N
            startup[n] = utils.startup_crash_rate(facets['startup_crash'])
            utils.analyze_platforms(platforms_prod[chan][sgn], facets['platform_pretty_version'])


def get_sgns_data_helper(data, platforms, signatures, bids_chan, extra, search_date, product, channel=None):
    """Get the data from Socorro and collect the stats.
    """
    limit = 80
    base_params = {'build_id': [utils.get_buildid(bid) for bid in bids_chan.keys()],
                   'date': search_date,
                   'product': product,
...
def get_sgns_data(channels, versions, platforms, signatures, extra, products, towait):
    ...
    for prod, bids in unique.items():
        towait.append(get_sgns_data_helper(data, platforms, signatures,
                                           bids, extra, search_date,
                                           prod))
...
def get_pushdates(chan_rev):
    ...
    for chan, revs in chan_rev.items():
        ...
        for rev in revs:
            res.append(hgmozilla.Revision(channel=chan,
                                          params={'node': rev},
